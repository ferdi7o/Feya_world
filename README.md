...
The project uses `python-dotenv` to manage sensitive information. Since the `.env` file is excluded from the repository for security reasons, you must create one manually:

1.  **Create a `.env` file** in the root directory (same level as `manage.py`).
2.  **Generate a Secret Key:** You can use your own key or generate a new one by running this command in your terminal:
    ```bash
    python -c 'from django.core.management.utils import get_random_secret_key; print(get_random_secret_key())'
    ```
3.  **Fill in the variables:** Ensure your `.env` looks like this:
    ```env
    DEBUG=True
